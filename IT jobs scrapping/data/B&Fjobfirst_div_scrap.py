...
div_data = []
respond = requests.get("https://www.timesjobs.com/candidate/job-search.html?searchType=Home_Search&from=submit&asKey=OFF&txtKeywords=&cboPresFuncArea=42",headers=headers)
source  = respond.text

soup_data = BeautifulSoup(source,'html.parser')

second_page = requests.get("https://www.timesjobs.com/job-detail/branch-service-manager-tekpillar-bengaluru-bangalore-davanagere-dharwad-hubli-mangalore-1-to-6-yrs-jobid-JfABHiRvpdtzpSvf__PLUS__uAgZw==&source=srp").text
...
company_name = first_div_inner_page.find('h2').text.strip().replace('Jobs by this Company', '').strip()
# print(company_name)
...
df.to_excel("B&Fjobfirst_div_scrap.xlsx")